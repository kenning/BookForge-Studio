    ...
    def forward(
        ...
        **kwargs,
        ...
        if is_using_cuda_graph:
            assert (
                decode_stage and use_static_cache
        ...
        if has_audio_out and not self.fast_forward:
            ...
            if decode_stage and is_using_cuda_graph:
                assert (
                    is_decoding_audio_token is not None
        ...
        if output_attentions:
            if self.use_audio_attention:
                ...
                outputs += (
                    torch.concat([self_attn_weights, audio_self_attn_weights], dim=1),
                )
            else:
...
class HiggsAudioModel(HiggsAudioPreTrainedModel, GenerationMixin):
    ...
    def __init__(self, config: HiggsAudioConfig):
        ...
        self.audio_out_bos_token_id = (
            ...
            if "audio_out_bos_token_id" in config
            else None
        )
    ...
    def forward(
        ...
        if (
            ...
            _forward_core = self.decode_graph_runners[
        ...
        hidden_states, all_hidden_states, all_self_attns = _forward_core(
            ...
            is_decoding_audio_token=(
                is_decoding_audio_token if use_static_cache else None
            ),
            cache_position=cache_position,
            ...
            fast_forward_attention_mask=(
                fast_forward_attention_mask if use_static_cache else None
            ),
            output_attentions=output_attentions,
    ...
    def _copy_kv_cache(self, from_cache: Cache, to_cache: Cache):
        ...
        for layer_idx in range(num_layers):
            ...
            assert (
                to_cache.get_max_cache_shape() >= from_cache_size
    ...
    def capture_model(
        ...
        for past_key_value in past_key_values:
            ...
            for is_decoding_audio_token in [True, False]:
                ...
                self.decode_graph_runners[kv_cache_length][
                    is_decoding_audio_token
                ] = runner
